...
> **Cheaper and simpler with the UPS HAT:** if you fit the Geekworm X1202/X1206 from the Optional table, skip the 27W USB-C supply. Any barrel-jack supply that gives the UPS enough power feeds it (at least 36W anywhere in its 6-18V range: 3A at 12V, 4A at 9V, 2A at 18V; see the adapter row in the Optional table), and the UPS delivers 5.1V 5A to the Pi over its pogo pins; the Geekworm setup script sets `PSU_MAX_CURRENT=5000` so the Pi treats it as a 5A supply and keeps the full USB budget. For a device that lives in a case, the DC barrel jack is the better input either way: there is no USB PD negotiation to fail and no USB-C extension to sag, and a 12V adapter you already own will do.
...
| **DC adapter for the X1202 (5.5 × 2.1 mm barrel, center positive, 6-18V, at least 36W)** | Feeds the X1202 through its barrel jack instead of USB-C, which is the better input for a cased build: no USB PD negotiation to fail and no USB-C extension to sag. The X1202 accepts 6-18V DC on that jack and converts it to the 5.1V 5A the Pi 5 needs while also charging the cells. **What matters is power, not the voltage on its own.** Geekworm's baseline is 3A at 12V, about 36W, and an adapter at any other voltage in the range has to deliver the same 36W, so the current it must be rated for rises as its voltage falls: 18V → 2A, 15V → 2.4A, 12V → 3A, 9V → 4A, 6V → 6A. A 12V adapter rated below 3A, or a 9V adapter rated at 3A, will sag or brown out when the UPS is charging with both radars running. Below about 9V the current needed passes what a 5.5 × 2.1 mm barrel plug is usually rated for (5A), so treat 9-18V as the usable range; 12V is the common choice and what the examples use. Examples: MEAN WELL GST36 (12V 3A; GST36U12-P1J US plug, GST36E12-P1J EU plug, both at Mouser) or Geekworm's own PSU60 (12V 5A, also sold as an Amazon bundle with the X1202). Never connect the DC jack and the USB-C input at the same time | [Mouser (EU plug)](https://www.mouser.com/en/ProductDetail/MEAN-WELL/GST36E12-P1J) / [Mouser (US plug)](https://www.mouser.com/c/?q=GST36U12-P1J) / [Amazon (PSU60)](https://www.amazon.com/dp/B0BDF89DCB) | ~$15 |
